#!/usr/bin/env bash
# Pipeline variant for local_clients Parquet (uses learning/map_local_clients_parquet.py).
# Called by run_local_clients_from_minio.sh — or run directly after setup-minio-env.sh.
...
S3_URI="${S3_URI:-s3://senzing-incoming/local_clients/}"
INCOMING_DIR="${INCOMING_DIR:-local_clients}"
PARQUET_FILE="${PARQUET_FILE:-local_clients.parquet}"
JSONL_FILE="${JSONL_FILE:-mapped_local_clients_pq.jsonl}"
DATA_SOURCE="${SENZING_DATA_SOURCE:-LOCAL_CLIENTS_PQ}"
...
docker run --rm \
  ...
  'pip -q install pandas pyarrow && python learning/map_local_clients_parquet.py'
